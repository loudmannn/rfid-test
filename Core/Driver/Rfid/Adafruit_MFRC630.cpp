...
/*!
 * @brief Uses the lookup table above to reverse a single int8_t.
 * @param n Input int8_t
 * @return uint8_t. A int8_t
 */
...
/**************************************************************************/
/*!
    @brief  Write a int8_t to the specified register
*/
/**************************************************************************/
void Adafruit_MFRC630::write8(int8_t reg, int8_t value) {
  TRACE_TIMESTAMP();
  ...
  // case MFRC630_TRANSPORT_I2C:
    ...
    HAL_StatusTypeDef ret = HAL_I2C_Master_Transmit(_i2c, _i2c_addr, txBuffer, 2, TIMEOUT);
    if(ret != HAL_OK)
        return;
    // break;
  // default:
...
*/
/**************************************************************************/
void Adafruit_MFRC630::writeBuffer(int8_t reg, uint16_t len, uint8_t *buffer) {
  TRACE_TIMESTAMP();
  TRACE_PRINT(F("Writing "));
  TRACE_PRINT(len);
  TRACE_PRINT(F(" int8_t(s) to 0x"));
  TRACE_PRINTLN(reg, HEX);
  ...
  // case MFRC630_TRANSPORT_I2C:
    ...
    HAL_StatusTypeDef ret = HAL_I2C_Master_Transmit(_i2c, _i2c_addr, txBuffer, 1, TIMEOUT);
    if(ret != HAL_OK)
      return;
    else
    {
      ret = HAL_I2C_Master_Transmit(_i2c, _i2c_addr, buffer, len, TIMEOUT);
      if(ret != HAL_OK)
        return;
    }
  //   break;
...
/**************************************************************************/
/*!
    @brief  Read a int8_t from the specified register
*/
/**************************************************************************/
int8_t Adafruit_MFRC630::read8(int8_t reg) {
  uint8_t resp = 0;
  uint8_t tx[2] = {0};
  uint8_t rx[2] = {0};
  ...
  TRACE_TIMESTAMP();
  TRACE_PRINT(F("Requesting 1 int8_t from 0x"));
  TRACE_PRINTLN(reg, HEX);
  ...
  // case MFRC630_TRANSPORT_I2C:
    ...
    {
      ret = HAL_I2C_Master_Receive(_i2c, _i2c_addr, rxBuffer, 1, TIMEOUT);
      if(ret != HAL_OK)
        resp = 0;
      else
        resp = rxBuffer[0];
...
*/
/**************************************************************************/
Adafruit_MFRC630::Adafruit_MFRC630(I2C_HandleTypeDef *i2cBus, uint8_t i2c_addr, GPIO_TypeDef* pdown_port, uint16_t pdown_pin) {
  /* Set the transport */
  _transport = MFRC630_TRANSPORT_I2C;

  /* Set the PDOWN pin */
  _pdown = true;
  _pdown_port = *pdown_port;
  _pdown_pin = pdown_pin;
...
//   }
  ...
  if (_pdown) {
    ...
    HAL_GPIO_WritePin(&_pdown_port, _pdown_pin, GPIO_PIN_SET);
    HAL_GPIO_WritePin(&_pdown_port, _pdown_pin, GPIO_PIN_RESET);
  ...
  int8_t ver = read8(MFRC630_REG_VERSION);
...
/**************************************************************************/
/*!
    @brief  Determines the number of int8_ts in the HW FIFO buffer (max 512)

    @returns The number of int8_ts available in the HW FIFO buffer
*/
/**************************************************************************/
int16_t Adafruit_MFRC630::readFIFOLen(void) {
  ...
  /* In 512 int8_t mode, the upper two bits are stored in FIFO_CONTROL */
  int8_t hi = read8(MFRC630_REG_FIFO_CONTROL);
  int8_t lo = read8(MFRC630_REG_FIFO_LENGTH);

  /* Determine len based on FIFO size (255 int8_t or 512 int8_t mode) */
  int16_t l = (hi & 0x80) ? lo : (((hi & 0x3) << 8) | lo);

  DEBUG_TIMESTAMP();
  DEBUG_PRINT(F("FIFO contains "));
  DEBUG_PRINT(l);
  DEBUG_PRINTLN(F(" int8_t(s)"));
...
/**************************************************************************/
/*!
    @brief  Read 'len' int8_ts from the HW FIFO buffer (max 512 int8_ts)

    @returns The number of int8_ts read from the FIFO, or -1 if an error occured.
*/
/**************************************************************************/
int16_t Adafruit_MFRC630::readFIFO(uint16_t len, uint8_t *buffer) {
  int16_t counter = 0;

  /* Check for 512 int8_t overflow */
  if (len > 512) {
    return -1;
  }
  ...
  DEBUG_PRINT(len);
  DEBUG_PRINTLN(F(" int8_t(s) from FIFO"));

  /* Read len int8_ts from the FIFO */
...
/**************************************************************************/
/*!
    @brief  Writes the specified number of int8_ts to the HW FIFO

    @returns The number of int8_ts written to the FIFO, -1 if an error occured.
*/
/**************************************************************************/
int16_t Adafruit_MFRC630::writeFIFO(uint16_t len, uint8_t *buffer) {
  ...
  DEBUG_PRINT(len);
  DEBUG_PRINTLN(F(" int8_t(s) to FIFO"));

  /* Write len int8_ts to the FIFO */
  for (uint16_t i = 0; i < len; i++) {
...
*/
/**************************************************************************/
void Adafruit_MFRC630::writeCommand(int8_t command) {
...
*/
/**************************************************************************/
void Adafruit_MFRC630::writeCommand(int8_t command, uint8_t paramlen,
                                    uint8_t *params) {
  /* Arguments and/or data necessary to process a command are exchanged via
     ...
     - The FIFO buffer is not cleared automatically at command start. It is
       recommended to write the command arguments and/or the data int8_ts into
...
/**************************************************************************/
/*!
    @brief  Prints out n int8_ts of hex data.
*/
/**************************************************************************/
...
uint16_t Adafruit_MFRC630::iso14443aCommand(enum iso14443_cmd cmd) {
  uint16_t atqa = 0; /* Answer to request (2 int8_ts). */
  ...
  /*
   * Define the number of bits from the last int8_t should be sent. 000 means
   * that all bits of the last data int8_t are sent, 1..7 causes the specified
  ...
  if (rxlen == 2) {
    /*
     * If we have 2 int8_ts for the response, it's the ATQA.
     *
...
uint8_t Adafruit_MFRC630::iso14443aSelect(uint8_t *uid, uint8_t *sak) {
  ...
  for (cascadelvl = 1; cascadelvl <= 3; cascadelvl++) {
    ...
    for (cnum = 0; cnum < 32; cnum++) {
      ...
      /* Read the UID so far */
      uint16_t rxlen = readFIFOLen();
      uint8_t buf[5]; /* UID = 4 int8_ts + BCC */
    ...
    /* Reset the TX and RX registers (disable alignment, transmit full int8_ts) */
    write8(MFRC630_REG_TX_DATA_NUM, (kbits % 8) | (1 << 3));
    uint8_t rxalign = 0;
    write8(MFRC630_REG_RX_BIT_CTRL, (0 << 7) | (rxalign << 4));
    ...
    if (sak_value & (1 << 2)) {
      ...
      /* Done! */
      /* Add current int8_ts at this level to the UID. */
      uint8_t UIDn;
...
uint16_t Adafruit_MFRC630::mifareWriteBlock(uint16_t blocknum, uint8_t *buf) {
  ...
  /* We should have a single ACK int8_t in buffer at this point. */
  uint16_t buffer_length = readFIFOLen();
  if (buffer_length != 1) {
    DEBUG_TIMESTAMP();
